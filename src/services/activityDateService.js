...
export async function fetchActivityDates(activityIds, { activeOnly = true } = {}) {
  ...
  let query = supabase.from('activity_dates').select('id, season_id, activity_date, sort_order').in('season_id', ids)
  if (activeOnly) query = query.eq('is_active', true)

  const { data, error } = await query.order('season_id', { ascending: true }).order('sort_order', { ascending: true })
...
export async function fetchActivityDatesByIds(activityDateIds) {
  ...
  const { data, error } = await supabase.from('activity_dates').select('id, season_id, activity_date, sort_order').in('id', ids)
  if (error) throw error
  return data || []
}
...
export async function fetchActivityDatesInRange(startDate, endDate) {
  const { data, error } = await supabase.from('activity_dates').select('id, season_id, activity_date, sort_order').gte('activity_date', startDate).lt('activity_date', endDate)
  if (error) throw error
  return data || []
}
